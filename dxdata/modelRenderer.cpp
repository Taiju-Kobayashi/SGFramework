...
void ModelRenderer::Draw()
{
	...
	UINT offset = 0;
	Render.GetDeviceContext()->IASetVertexBuffers(0, 1, &m_model->VertexBuffer, &stride, &offset);

	// インデックスバッファ設定
	Render.GetDeviceContext()->IASetIndexBuffer(m_model->IndexBuffer, DXGI_FORMAT_R32_UINT, 0);
	...
	{
		// マテリアル設定
		Render.SetMaterial(m_model->SubsetArray[i].Material.Material);

		// テクスチャ設定
		if (m_model->SubsetArray[i].Material.Texture)
			Render.GetDeviceContext()->PSSetShaderResources(0, 1, &m_model->SubsetArray[i].Material.Texture);

		// ポリゴン描画
		Render.GetDeviceContext()->DrawIndexed(m_model->SubsetArray[i].IndexNum, m_model->SubsetArray[i].StartIndex, 0);
	}
...
void ModelRenderer::LoadModel(const char* FileName, MODEL* Model)
{
	...
	MODEL_OBJ modelObj;
	LoadObj(FileName, &modelObj, &Model->ModelSize);
	Model->HalfSize.x = Model->ModelSize.x * 0.5f;
	Model->HalfSize.y = Model->ModelSize.y * 0.5f;
	Model->HalfSize.z = Model->ModelSize.z * 0.5f;
	...
	// 頂点バッファ生成
	{
		...
		Render.GetDevice()->CreateBuffer(&bd, &sd, &Model->VertexBuffer);
	}
	...
	// インデックスバッファ生成
	{
		...
		Render.GetDevice()->CreateBuffer(&bd, &sd, &Model->IndexBuffer);
	}
	...
	// サブセット設定
	{
		...
		for (unsigned int i = 0; i < modelObj.SubsetNum; i++)
		{
			...
			auto hr = LoadFromDDSFile(wc, DDS_FLAGS_NONE, &metadata, image);
			if (FAILED(hr))
				LoadFromWICFile(wc, WIC_FLAGS_NONE, &metadata, image);
			CreateShaderResourceView(Render.GetDevice(), image.GetImages(), image.GetImageCount(), metadata, &Model->SubsetArray[i].Material.Texture);
...
//モデル読込////////////////////////////////////////////
void ModelRenderer::LoadObj(const char* FileName, MODEL_OBJ* ModelObj, XMFLOAT3* ModelSize)
{
	...
	fclose(file);
	XMFLOAT3 size	= CalculateAABB(ModelObj->VertexArray, ModelObj->VertexNum);
...
XMFLOAT3 ModelRenderer::CalculateAABB(const VERTEX_3D* positions, unsigned int positionNum) {
	//std::numeric_limits<float>::max()とmax(a,b)の名前が被った故応急処置
	XMFLOAT3 minV(100000.0f, 100000.0f, 100000.0f);
	XMFLOAT3 maxV(std::numeric_limits<float>::lowest(), std::numeric_limits<float>::lowest(), std::numeric_limits<float>::lowest());
	XMFLOAT3 minPos, maxPos;

	for (unsigned int cnt = 0; cnt < positionNum; cnt++) {
		XMFLOAT3 pos{};
...
XMFLOAT3 ModelRenderer::GetSize() {
	XMFLOAT3 defalut{};
	if (m_model) {
		return m_model->ModelSize;
	}
	return defalut;
}


XMFLOAT3 ModelRenderer::GetHarfSize() {
	XMFLOAT3 defalut{};
	if (m_model) {
		return m_model->HalfSize;
	}
	return defalut;
}