...
void IlustTitle::Init(){
	...
	m_Audio->Init();
	m_Audio->Load("asset\\sound\\wafu.wav");
	m_Audio->Play(true);
	...
	ScratchImage image;
	LoadFromWICFile(L"asset\\texture\\titletextGM31.png", WIC_FLAGS_NONE, &metadata, image);
...
void IlustTitle:: PosInit(XMFLOAT2 Size, XMFLOAT2 Pos) {
	VERTEX_3D vertex[4];
...
void IlustTitle::Draw(){
	...
	auto tex = Render.GetDepthShadowTexture();
	Render.GetDeviceContext()->PSSetShaderResources(1, 1, &m_texture);
