...
void ReplayStone::Init() {
	...
	if (!instance->m_choicestone_path)
		((AnimationModel*)m_modelrenderer)->Load_NoBone("asset\\model\\stone.fbx");
	else {
		((AnimationModel*)m_modelrenderer)->Load_NoBone(instance->m_choicestone_path);
	}
...
void ReplayStone::Update() {
	...
	//摩擦
	m_velocity.x -= (m_velocity.x * 0.5f) * dt;
	m_velocity.z -= (m_velocity.z * 0.5f) * dt;

	m_rotation_speed.x -= (m_rotation_speed.x * 0.5f) * dt;
	m_rotation_speed.y -= (m_rotation_speed.y * 0.5f) * dt;
	m_rotation_speed.z -= (m_rotation_speed.z * 0.5f) * dt;
	...
	// 戦艦の大きさに応じた3点のオフセットを定義
	// これらの値は、戦艦の長さや幅に合わせて調整してください
	// 例: 戦艦の長さを20、幅を8とする場合
	...
	// もしそうでなければ、以前に議論した「3点から平面の法線を計算する」ロジックをこの部分に導入する必要があります。
	...
	// 衝突判定は速度が速い場合のみ行う
	if (XMFloat3Magnitude(m_velocity) > 10.0f) {
		...
		if (fabs(length) < rad) {
			...
			float rotationImpulseStrength = 0.002f; // 要調整
	...
	if (_field_check) {
		...
		float GroundHeight = _field_check->GetHeight(position) + (mscale.y * 0.5f);

		if (position.y <= GroundHeight) {
			position.y = GroundHeight; // 波の表面にY座標を固定
			m_velocity.y = 0.0f;       // Y速度をゼロに
			m_release = false; // 石のリリースフラグをオフにする
			m_IsEnd = true;//終了判定
		}
	}
...
void ReplayStone::RestartPlay() {
	auto ins = Manager::GetManager().GetGameInstance<ResultInstance>();
	m_IsEnd = false;
	m_release = false;
	m_transform->SetPosition(m_start_position);
	m_transform->SetRotation(ins->m_rotate);
	m_old_position = m_transform->GetPosition();
	AddForce(ins->m_stoneforce);
	AddRotForce(ins->m_rotate);
	SetReleaseFlag(true);
}


void ReplayStone::AddForce(const XMFLOAT3& force) {
	m_velocity = XMFloat3Add(m_velocity, force);
}

void ReplayStone::AddRotForce(const XMFLOAT3& force) {
